...
for input_file in glob.glob(os.path.join(path_to_folder, '*.*')):
    ...
    if input_file.split('.')[1] == 'csv':
        with open(input_file, 'r', newline='') as csv_in_file:
            ...
            inputfile.split('.')[1] == 'xlsx':
        workbook = oepn_workbook(input_file)
            ...
            for row in range(1, worksheet.nrows):
                ...
                for column in range(len(header)):
                    if worksheet.cell_type(row, column) == 3:
                        cell_value = \
                                xldate_as_tuple(worksheet.cell(row, column))\
                                .value, workbook.datemode)
                        cell_value = str(date(*cell_value[0:3])).strip()
                        row_of_output.append(cell_value)
                    else:
                        cell_value = \
                                str(worksheet.cell_value(row, column)).strip()
                        row_of_output.append(cell_value)
                row_of_output.append(os.path.basename(input_file))
                row_of_output.append(worksheet.name)
                if str(worksheet.cell(row, 0).value).split('.')[0].strip() \
...
print('Number of item numbers: ', count_of_numbers)
